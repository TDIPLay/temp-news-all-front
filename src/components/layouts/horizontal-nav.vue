    ...
    <b-container fluid>
      <nav class="navbar navbar-light navbar-expand-lg topnav-menu active">
        <div class="collapse navbar-collapse active" id="topnav-menu-content">
          <ul class="navbar-nav">
            <!-- Menu data -->
            <template v-for="(item, index) of customMenuItems" :key="index">
...
const customMenuItems = ref(menuItems);
...
const onMenuClick = (event: Event) => {
  ...
  const nextEl = (event.target as HTMLElement).nextSibling;
  if (nextEl && !(nextEl as HTMLElement).classList.contains("show")) {
    const parentEl = (event.target as HTMLElement).parentNode;
    if (parentEl) {
      (parentEl as HTMLElement).classList.remove("show");
    }
    (nextEl as HTMLElement).classList.add("show");
  } else if (nextEl) {
    (nextEl as HTMLElement).classList.remove("show");
  }
  return false;
};
