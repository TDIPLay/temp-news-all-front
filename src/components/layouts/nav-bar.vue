      ...
      <div class="d-flex">
        ...
        <div class="navbar-brand-box">
          <router-link to="/" class="logo logo-dark">
            <span class="logo-sm">
              <img src="@/assets/images/logo/main_logo.png" alt width="70" />
            </span>
            <span class="logo-lg">
              <img
          ...
          <router-link to="/" class="logo logo-light">
            <span class="logo-sm">
              <img
                src="@/assets/images/logo/main_logo_header_white.png"
                alt
                height="22"
              />
            </span>
            <span class="logo-lg">
              <img
                src="@/assets/images/logo/main_logo_white.png"
                alt
                width="70"
              />
            </span>
...
const lang = ref("kr");
const lan = ref(i18n.global.locale.value);
const text = ref(null);
const flag = ref("");
const languages = [
  {
...
onMounted(() => {
  if (process.env.VUE_APP_I18N_LOCALE) {
    flag.value = `${
      i18n.global.locale.value || process.env.VUE_APP_I18N_LOCALE
    }`;

    const headerLengImgEl = document.getElementById(
      "header-lang-img"
    ) as HTMLElement;

    if (!headerLengImgEl) return;
    languages.forEach((item) => {
      if (item.language == flag.value) {
        headerLengImgEl.setAttribute("src", item.flag);
      }
    });
  }
});
...
const setLanguage = (locale: string, country: any, flagSrc: any) => {
  lan.value = locale;
  text.value = country;
  flag.value = flagSrc;
  document.getElementById("header-lang-img")?.setAttribute("src", flagSrc);
